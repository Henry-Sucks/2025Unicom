...
from .gen_dependency_tree import get_semantic_dependencies
import re
from lxml import etree

class BugProcessor:
    def __init__(self, app_name, log_path, error_log_path, task, raw_solution, apis_path, api_xpath_file):
    ...
    def reorganize_log(self):
        ...
        for record_id, ui_record in enumerate(records):
            ...
            if (record_id == 0 and ui_record['Action'] == 'go back') or (record_id == 1 and ui_record['Action'] == 'scroll DOWN'):
                continue
            ...
            if ui_record['Choice'] is not None:
                ...
                if isinstance(choice_id, int):
                    ...
                    target_element_desc = etree.tostring(element, method="html", pretty_print=True).decode('utf-8')
                    action_desc = self.get_action_desc(ui_record['Action'], target_element_desc, ui_record['Input'])
                else:
                    action_desc = self.get_action_desc(ui_record['Action'], selected_element=choice_id, input_text=ui_record['Input'])
            else:
                action_desc = self.get_action_desc(ui_record['Action'], selected_element=None, input_text=ui_record['Input'])

            log_str += f"Detailed UI action: {action_desc}\n\n"

            reorganized_records.append({'ui_apis': ui_apis_ordered, 'ui_action': ui_record['Action']})
...
class BugProcessorv2:
    ...
    def get_commented_script(self):
        ...
        code = tools.get_combined_code('data/notes_preparation.txt', self.raw_solution)
        ...
        for lineno, comment in script_comments.items():
            ...
            code_dict[lineno] = f'{leading_spaces}{_get_formatted_comment(comment)}\n{code_dict[lineno]}\n'

        commented_code = '\n'.join([code_dict[i] for i in range(len(code_dict))])
        commented_code = tools.get_code_without_prefix('data/notes_preparation.txt', commented_code)
...
**Note that the script is a string of python code and should only output the JSON content.**'''
    ...
    def process_bug(self, prompt_answer_path, enable_dependency=False, model_name='gpt-3.5-turbo', stuck_ui_apis=None):
        prompt = self.make_prompt(enable_dependency=enable_dependency, stuck_ui_apis=stuck_ui_apis)
        answer = tools.query_gpt(prompt=prompt, model=model_name)
        tools.dump_json_file(prompt_answer_path, {'prompt': prompt, 'answer': answer})
        answer = tools.convert_gpt_answer_to_json(answer, model_name=model_name, default_value={'Plan': '', 'Script': ''})
