        ...
        for _, v in self.apis_data.items():
            for e in v:
                ...
                if enable_dependency:
                    dep = e['dependency']
                    semantic_dep = self.get_semantic_dependencies(dep)
                desc = e["desc"]
                func = e["func"]
...
'''
    ...
    def get_solution(self, app_name, prompt_answer_path, task, ui_elements, enable_dependency=False, model_name='gpt-3.5-turbo'):
        formatted_apis = self.format_all_apis(enable_dependency)
        prompt = self.make_prompt(task=task, app_name=app_name, state_ui_elements=ui_elements)
