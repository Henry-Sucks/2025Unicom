...
class InputEvent(object):
    ...
    def from_dict(event_dict):
        ...
        elif event_type == KEY_RenderEvent:
            return RenderEvent(event_dict=event_dict)
...
class ScrollEvent(UIEvent):
    ...
    def get_scroll_coordinates(self, device):
        if self.view is not None:
            from .device_state import DeviceState
        ...
        if self.direction.lower() == "up":
            ...
            start_x += width * 2 / 5
            end_x -= width * 2 / 5
        elif self.direction.lower() == "page_up":
            start_y = 300
            end_y = height - 300
        elif self.direction.lower() == "page_down":
            start_y = height - 300 
            end_y = 300
        elif self.direction.lower() == "page_right":
            start_x = 50 
            end_x = width - 100
        elif self.direction.lower() == "page_left":
            start_x = width - 100
            end_x = 50

        return start_x, start_y, end_x, end_y
    def send(self, device):
        start_x,start_y,end_x,end_y = self.get_scroll_coordinates(device)
        duration = 500
...
class SetTextEvent(UIEvent):
    ...
    def get_event_str(self, state):
        ...
        elif self.text is not None and self.y is not None:
            return "%s(state=%s, x=%s, y=%s, text=%s)" %\
                   (self.__class__.__name__, state.state_str, self.x, self.y, self.text)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
...
class RenderEvent(UIEvent):
    """
    render the element selection
    """

    def __init__(self, x=None, y=None, view=None, event_dict=None):
        super().__init__()
        self.event_type = KEY_RenderEvent
        self.x = x
        self.y = y
        self.view = view
        if event_dict is not None:
